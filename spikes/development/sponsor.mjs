...
export async function initializeSponsor(directory, port) {
  ...
  execFileSync('/usr/bin/openssl', ['req', '-x509', '-newkey', 'rsa:2048', '-nodes', '-days', '30',
    '-config', opensslConfig, '-keyout', join(directory, 'tls-key.pem'), '-out', join(directory, 'tls-cert.pem')],
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  try {
    ...
    if (action === 'init') console.log(JSON.stringify(await initializeSponsor(directory, Number(option))));
