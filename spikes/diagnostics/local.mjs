...
const components = Object.freeze({
  engine: ['ENGINE_STARTED', 'ENGINE_CLOSED', 'RECORDING_ENABLED', 'RECORDING_DISABLED', 'OPERATION_FROZEN', 'OPERATION_CANCELLED',
    ...
    'MESSAGE_APPEARANCE_RECORDED', 'CAPTURE_GAP', 'REQUEST_DEDUPLICATED', 'ENGINE_CAPTURE_DISABLED',
    'CAPTURE_SESSION_FAILED', 'CAPTURE_RECONCILIATION_FAILED', 'CAPTURE_TEXT_WRITE_FAILED', 'CAPTURE_DESCRIPTOR_WRITE_FAILED',
    'CAPTURE_ALIAS_WRITE_FAILED', 'CAPTURE_FAILURE_LOCKED', 'CAPTURE_FAILURE_UNRECOVERABLE', 'CAPTURE_FAILURE_CONFLICT',
    'CAPTURE_FAILURE_LIMIT', 'CAPTURE_FAILURE_INVALID', 'CAPTURE_FAILURE_INCOMPLETE', 'CAPTURE_FAILURE_UNSUPPORTED',
    'CAPTURE_FAILURE_STORAGE_BUSY', 'CAPTURE_FAILURE_STORAGE_FULL', 'CAPTURE_FAILURE_STORAGE_ACCESS',
    'CAPTURE_FAILURE_STORAGE_IO', 'CAPTURE_FAILURE_SQLITE', 'CAPTURE_FAILURE_TYPE', 'CAPTURE_FAILURE_RANGE', 'CAPTURE_FAILURE_UNEXPECTED'],
  vault: ['VAULT_CAPTURED', 'VAULT_STATE_COMMITTED', 'VAULT_WRITE_FAILED', 'VAULT_CAPACITY_EXHAUSTED'],
  ...
  adapter: ['SOURCE_FOLLOWED', 'SCOPE_ENROLLED', 'SCOPE_INVALIDATED', 'SCOPE_DESTINATION_CHANGED', 'CAPABILITY_UNAVAILABLE',
    ...
    'TRANSPORT_OBSERVER_READY', 'TRANSPORT_OBSERVER_WRAPPED', 'TRANSPORT_OBSERVER_REPLACED', 'TRANSPORT_OBSERVER_UNAVAILABLE',
    'TRANSPORT_RELAY_READY', 'TRANSPORT_RELAY_UNAVAILABLE', 'TRANSPORT_POLICY_READY', 'TRANSPORT_POLICY_UNAVAILABLE', 'TRANSPORT_POLICY_OFF',
    'REQUEST_NOT_OBSERVED', 'REQUEST_MATCHED', 'REQUEST_EXTRACTOR_REJECTED', 'REQUEST_BODY_READ_FAILED', 'REQUEST_BODY_LIMIT', 'REQUEST_JSON_INVALID',
  ...
  'REQUEST_PROMPT_INVALID', 'REQUEST_MEDIA_IGNORED', 'REQUEST_CONVERSATION_UNAVAILABLE', 'REQUEST_CONVERSATION_DIFFERENT', 'REQUEST_MESSAGE_REJECTED',
    'REQUEST_MESSAGE_MISSING', 'DURABLE_SAVE_DISPATCHED'],
  anchor: ['SPONSOR_REQUESTED', 'SPONSOR_SUBMITTED', 'SPONSOR_UNAVAILABLE', 'ACCOUNT_REQUIRED',
    ...
    'ANCHOR_RETRY_SCHEDULED', 'ANCHOR_RETRY_STARTED', 'CONFIRMATION_PENDING', 'CONFIRMATION_REJECTED', 'CONFIRMATION_ACCEPTED', 'CONSENSUS_UPGRADED'],
});
...
// Only fixed error classes cross this boundary. Messages, paths, stack traces
// and unknown codes may contain evidence or credentials and are never recorded.
const captureFailures = new Map([
  ['VAULT_CAPACITY_EXHAUSTED', 'VAULT_CAPACITY_EXHAUSTED'],
  ['LOCKED', 'CAPTURE_FAILURE_LOCKED'], ['UNRECOVERABLE', 'CAPTURE_FAILURE_UNRECOVERABLE'],
  ['CONFLICT', 'CAPTURE_FAILURE_CONFLICT'], ['LIMIT_EXCEEDED', 'CAPTURE_FAILURE_LIMIT'],
  ['INVALID', 'CAPTURE_FAILURE_INVALID'], ['INCOMPLETE', 'CAPTURE_FAILURE_INCOMPLETE'],
  ['UNSUPPORTED', 'CAPTURE_FAILURE_UNSUPPORTED'], ['ENOSPC', 'CAPTURE_FAILURE_STORAGE_FULL'],
  ['EACCES', 'CAPTURE_FAILURE_STORAGE_ACCESS'], ['EPERM', 'CAPTURE_FAILURE_STORAGE_ACCESS'],
  ['EROFS', 'CAPTURE_FAILURE_STORAGE_ACCESS'], ['EIO', 'CAPTURE_FAILURE_STORAGE_IO'],
]);
const sqliteCaptureFailures = new Map([
  [5, 'CAPTURE_FAILURE_STORAGE_BUSY'], [6, 'CAPTURE_FAILURE_STORAGE_BUSY'],
  [8, 'CAPTURE_FAILURE_STORAGE_ACCESS'], [10, 'CAPTURE_FAILURE_STORAGE_IO'],
  [13, 'CAPTURE_FAILURE_STORAGE_FULL'],
]);
export function emitCaptureFailure(diagnostics, error, data = {}) {
  let code = 'CAPTURE_FAILURE_UNEXPECTED';
  try {
    if (error?.code === 'ERR_SQLITE_ERROR') {
      code = sqliteCaptureFailures.get(error.errcode) ?? 'CAPTURE_FAILURE_SQLITE';
    } else code = captureFailures.get(error?.code)
      ?? (error instanceof TypeError ? 'CAPTURE_FAILURE_TYPE'
        : error instanceof RangeError ? 'CAPTURE_FAILURE_RANGE' : code);
  } catch {}
  emit(diagnostics, code, data);
}
