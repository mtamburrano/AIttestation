...
function showRecording(state) {
  ...
  const labels = { READY: 'Attestamp · Continuous', SAVING: 'Attestamp · Saving prompt…',
...
function observeSend(event, inputMethod) {
  ...
  if (!event.isTrusted || !policy || !policyCurrent(policy) || activeRelease || document.visibilityState !== 'visible') return;
