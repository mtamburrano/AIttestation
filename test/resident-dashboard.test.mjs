...
async function fixture(t, options = {}) {
  ...
  f = await recordingFixture(root, { recording: true, installation, ...options });
...
test('dashboard close/reopen, pause and account loss retain prompt counts, selective export and recovery', async t => {
  ...
  state = await api('/dashboard/state'); assert.equal(state.integration.code, 'OFF');
  assert.equal(state.integration.healthy, false); assert.equal(state.recording, false);
...
test('reversible integration changes invalidate scopes and preserve preferences, keys and unrelated configuration', async t => {
  ...
  await f.recording(true); f.send('RETAINED_AFTER_REMOVAL');
  await until(() => f.runtime.session.receipts.list().length === 1); await f.runtime.engine.drain();
  const history = (await api('/dashboard/state')).history;
  const preferences = f.runtime.engine.state().recording;
  ...
  assert.deepEqual(f.runtime.engine.state().recording, preferences);
...
test('prompt summaries preserve cancellation and uncertainty without counting internal or derivative records', () => {
  ...
  const state = { available: true, scopes: [{ effectiveRecording: 'UNAVAILABLE' }], recording: true, capabilities: { observation: false } };
  assert.equal(integrationStatus(state, { integration: 'ENABLED' }, true).healthy, false);
  assert.equal(integrationStatus({ ...state, scopes: [] }, { integration: 'ENABLED' }, true).code, 'WAITING_FOR_TABS');
});

test('disabling during confirmation permits durable evidence anchoring without deleting the pending prompt', async t => {
  let complete;
  const confirmation = new Promise(resolve => { complete = resolve; });
  t.after(() => complete({ synthetic: true }));
  const { f, api } = await fixture(t, { collectFast: () => confirmation });
  f.send('PENDING_SYNTHETIC_PROMPT');
  ...
  assert.equal(state.history.counts.prompts, 1); assert.equal(state.history.prompts[0].state, 'PROMPT_SAVED');
...
test('resident pipe sends content-free authoritative status and rejects stale controls', async t => {
  ...
  const send = value => { const body = Buffer.from(JSON.stringify({ profile: 'pap-desktop-command/2', ...value }));
  ...
  assert.equal(initial.code, 'SOURCES_READY');
  assert.doesNotMatch(JSON.stringify(initial), /fixture-17|"scope":|url|token|text|digest/i);
  send({ kind: 'RECORDING', runtimeEpoch: initial.runtimeEpoch, revision: initial.revision, enabled: false });
  await until(() => events.at(-1).recording === false); assert.equal(events.at(-1).code, 'OFF');
  send({ kind: 'RECORDING', runtimeEpoch: initial.runtimeEpoch, revision: initial.revision, enabled: true });
  await until(() => events.at(-1).code === 'ACTION_FAILED'); assert.equal(f.runtime.engine.state().recording, false);
...
test('launch stays quiet and simultaneous free-verifier requests share one server', async t => {
  ...
  assert.deepEqual(opened, [], 'startup keeps the dashboard closed');
