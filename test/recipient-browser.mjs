...
let chrome, socket, recipient, vault;
try {
  ...
  chrome = spawn('/Applications/Google Chrome.app/Contents/MacOS/Google Chrome', ['--headless=new', `--user-data-dir=${join(root, 'profile')}`,
    ...
    '--disable-default-apps', '--disable-updater-scheduler', '--use-mock-keychain', '--no-proxy-server', '--host-resolver-rules=MAP * ~NOTFOUND, EXCLUDE 127.0.0.1', 'about:blank'],
  ...
  const derivative = receipts.redact({ id: receipts.list()[0].id, text: 'Share this [REDACTED]' });
  const preview = receipts.prepare({ ids: [derivative.id] });
  assert.equal(preview.records.length, 1); assert.doesNotMatch(JSON.stringify(preview), /PRIVATE ORIGINAL/);
  const exported = join(root, 'synthetic-evidence.json');
  const bytes = receipts.export(preview.previewId); await writeFile(exported, bytes);
  const bundle = parseCanonical(bytes);
  assert.equal(unpack(bundle.disclosure.objects[0].bytes).toString(), 'Share this [REDACTED]');
  ...
  await setFile('#bundle', exported); await click('verify');
  await wait("document.querySelector('#status').textContent.startsWith('Local verification finished')");
  assert.equal(await evaluate('globalThis.ACTIVE_CONTENT === true'), false);
  ...
  console.log('PASS: independent verifier, redacted evidence, hostile input, recovery after rejection and no external requests.');
} finally {
  socket?.close();
  if (chrome && chrome.exitCode === null) {
    const stopped = new Promise(resolve => chrome.once('exit', resolve)); chrome.kill(); await stopped;
  }
  await recipient?.close(); vault?.close(); await rm(root, { recursive: true, force: true });
}
